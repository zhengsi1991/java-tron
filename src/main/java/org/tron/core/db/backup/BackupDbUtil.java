...
public class BackupDbUtil {
  ...
  public boolean canDoBackupOrSetDelay(BlockCapsule block) {
    //not reach frequency, but need judge whether delay one block time
    if (block.getNum() % args.getDbBackupConfig().getFrequency() != 0) {
      if (isDelay()) {
        setDelay(false);
        logger.info("backup db delay one block time.");
        return true;
      } else {
        return false;
      }
      ...
      if (block.generatedByMyself) {
        // reach frequency, but this block is generated by myself, delay one block time to do backup
        setDelay(true); // delay one block
        logger.info("set backup db delay one block time.");
        return false;
      }
      return true;
    }
  ...
  public void doBackup(BlockCapsule block) {
    if (!canDoBackupOrSetDelay(block)) {
      return;
    }
