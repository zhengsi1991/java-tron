...
@Slf4j
public class MultiSignAddKey024 {
  ...
  private String dev001Key2 = ByteArray.toHexString(ecKey2.getPrivKeyBytes());
  ...
  public void testMultiSignAddKey() {
    ...
    //1.address is not in permission keylist
    Assert.assertTrue(PublicMethed
    ...
    Return returnResult = PublicMethedForMutiSign
        .permissionUpdateKey2(permission, testAddress, 1, testAddress, dev001Key,
            blockingStubFull);
    Assert
    ...
    Assert
        .assertThat(returnResult.getMessage().toStringUtf8(),
            containsString("address is not in permission active"));
