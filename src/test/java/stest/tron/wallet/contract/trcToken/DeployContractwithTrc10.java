...
@Slf4j
public class DeployContractwithTrc10 {
...
//    Assert.assertTrue(PublicMethed.freezeBalanceForReceiver(fromAddress, 100000000L,
//        3, 1, ByteString.copyFrom(dev001Address), testKey002, blockingStubFull));
//    Assert.assertTrue(PublicMethed.freezeBalanceForReceiver(fromAddress, 1000000000L,
//        3, 1, ByteString.copyFrom(user001Address), testKey002, blockingStubFull));

    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(dev001Address, 1000_000_000L,
        3, 1, dev001Key, blockingStubFull));
    Assert.assertTrue(PublicMethed.freezeBalance(dev001Address, 100_000_000L,
        3, dev001Key, blockingStubFull));
    ...
    deployTokenBalanceContract(dev001Address, dev001Key);
    ...
    triggerTokenBalanceContract(tokenBalanceContractAddress, receiveTokenContractAddress,
        user001Address, user001Key);
    triggerTokenBalanceContract(tokenBalanceContractAddress, dev001Address, user001Address,
        user001Key);
  }
  ...
  public void deployTransferTokenContract(byte[] dev001Address, String dev001Key) {
    ...
    String transferTokenTxid = PublicMethed
        ...
        maxFeeLimit, 0L, 100, null, dev001Key,
    ...
    logger.info("balanceAfter is " + Long.toString(balanceAfter));

    //transfer Assetissue to this contract
    ...
    Long assetIssueCount = getAssetIssueValue(transferTokenContractAddress, assetAccountId, blockingStubFull);
    logger.info("contract has AssetId: " + assetAccountId.toStringUtf8() + ", Count: " + assetIssueCount);


    Assert.assertTrue(PublicMethed.transferAsset(user001Address,
        assetAccountId.toByteArray(), 100L, dev001Address, dev001Key, blockingStubFull));
    Long userAssetIssueCount = getAssetIssueValue(user001Address, assetAccountId, blockingStubFull);
    logger.info("user001Address has AssetId: " + assetAccountId.toStringUtf8() + ", Count: " + assetIssueCount);

    Assert.assertTrue(energyLimit > 0);
    Assert.assertTrue(energyUsage > 0);
    Assert.assertEquals(balanceBefore, balanceAfter);
    Assert.assertEquals(Long.valueOf(100), assetIssueCount);
    Assert.assertEquals(Long.valueOf(100), userAssetIssueCount);
 }


  public void deployRevContract(byte[] dev001Address, String dev001Key) {

    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(dev001Address, 1000000L,
  ...
  public void deployTokenBalanceContract(byte[] dev001Address, String dev001Key) {
    ...
    String code = "";
    ...
    String tokenBalanceTxid = PublicMethed.deployContractAndGetTransactionInfoById(contractName,
        abi, code, "", maxFeeLimit, 0L, 100, 20000,
        assetAccountId.toStringUtf8(), 2, null, dev001Key,
        dev001Address, blockingStubFull);

    Optional<TransactionInfo> infoById = PublicMethed
        .getTransactionInfoById(tokenBalanceTxid, blockingStubFull);

    if (infoById.get().getResultValue() != 0) {
      Assert.fail("deploy token balance failed with message: " + infoById.get().getResMessage());
    }
    tokenBalanceContractAddress = infoById.get().getContractAddress().toByteArray();

    SmartContract smartContract = PublicMethed.getContract(tokenBalanceContractAddress,
        blockingStubFull);
    Assert.assertNotNull(smartContract.getAbi());
    ...
    energyUsage = accountResource.getEnergyUsed();
    long balanceAfter = PublicMethed.queryAccount(dev001Key, blockingStubFull).getBalance();
    Long assetIssueCount = getAssetIssueValue(tokenBalanceContractAddress, assetAccountId, blockingStubFull);
    ...
    Assert.assertTrue(energyUsage > 0);
    Assert.assertEquals(balanceBefore, balanceAfter);
    Assert.assertEquals(Long.valueOf(2), assetIssueCount);
  }
  ...
  public void triggerContract(byte[] transferTokenContractAddress,
    ...
    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(user001Address, 500_000_000L,
    ...
    Long receiveAssetBefore = getAssetIssueValue(receiveTokenContractAddress, assetAccountId, blockingStubFull);
    ...
    Long transferAssetAfter = getAssetIssueValue(transferTokenContractAddress, assetAccountId, blockingStubFull);
    logger.info("after trigger, transferTokenContractAddress has AssetId "
        + assetAccountId.toStringUtf8() + ", Count is " + transferAssetAfter);

    Long receiveAssetAfter = getAssetIssueValue(receiveTokenContractAddress, assetAccountId, blockingStubFull);
    logger.info("after trigger, receiveTokenContractAddress has AssetId "
        + assetAccountId.toStringUtf8() + ", Count is " + receiveAssetAfter);
