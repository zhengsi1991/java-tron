...
@Slf4j
public class ContractOriginEnergyLimit038 {
...
//  }
  ...
  public void testOriginEnergyLimit() {
    ...
    long originEnergyLimit = 700;

    //dev balance and Energy
    long devTriggerTargetBalance = 0;
    long devTriggerTargetEnergy = 1000000000;

    // user balance and Energy
    long userTargetBalance = 0L;
    ...
    logger.info("==================================");
    long userAvailableEnergy = getUserAvailableEnergy(user001Address);
    ...
    Assert.assertTrue((devEnergyUsageBefore + originEnergyUsageAutual) >= devEnergyUsageBefore);
    Assert.assertTrue(userEnergyUsageAfter >= userEnergyUsageBefore);

    Assert.assertTrue(EnergyFeeAutual / 100 + userEnergyUsageAfter
        == userAvailableEnergy);
    logger.info("EnergyFeeAutual:" + EnergyFeeAutual);
    logger.info("userEnergyUsageAfter:" + userEnergyUsageAfter);
    logger.info("userAvailableEnergy:" + userAvailableEnergy);
