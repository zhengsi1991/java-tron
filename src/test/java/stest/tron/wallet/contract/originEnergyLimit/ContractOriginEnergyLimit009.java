...
@Slf4j
public class ContractOriginEnergyLimit009 {
  ...
  public void beforeSuite() {
    ...
    if (PublicMethed.queryAccount(freezeAddress, blockingStubFull).getBalance() > 10_000_000L) {
      Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(freezeAddress,
          PublicMethed.queryAccount(freezeAddress, blockingStubFull).getBalance() - 10_000_000,
...
//    logger.info("User feeLimit: " + getFeeLimit(txid)*10000);
  ...
  public void testOriginEnergyLimit() {
    ...
    long devTriggerTargetEnergy = 100;

    // user balance and Energy
    long userTargetBalance = 0;
    long userTargetEnergy = 0L;
    ...
    Assert.assertTrue(PublicMethed.sendcoin(dev001Address, devNeedBalance, fromAddress,
        testKey002, blockingStubFull));
    Assert.assertTrue(PublicMethed.sendcoin(dev001Address, 1000000, user001Address,
