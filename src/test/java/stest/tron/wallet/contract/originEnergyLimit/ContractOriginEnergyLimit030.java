...
@Slf4j
public class ContractOriginEnergyLimit030 {
...
//  }
  ...
  public void testOriginEnergyLimit() {
    ...
    long originEnergyLimit = 500;

    //dev balance and Energy
    long devTriggerTargetBalance = 0;
    long devTriggerTargetEnergy = 600;
...
//    DEV is  enough to pay
    ...
    Assert.assertEquals(userExpectCost, 592);
    Assert.assertEquals(devExpectCost, 0);
    long originEnergyUsageAutual = infoById.get().getReceipt().getOriginEnergyUsage();
    Assert.assertEquals(originEnergyUsageAutual, 0);
    long EnergyUsageAutual = infoById.get().getReceipt().getEnergyUsage();
    Assert.assertEquals(EnergyUsageAutual, 592);
