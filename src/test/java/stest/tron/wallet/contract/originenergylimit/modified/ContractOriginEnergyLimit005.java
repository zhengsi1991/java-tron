package stest.tron.wallet.contract.originenergylimit.modified;
...
@Slf4j
public class ContractOriginEnergyLimit005 {
  ...
  protected String fullnode = Configuration.getByPath("testng.conf")
      .getStringList("fullnode.ip.list").get(0);
  private String fullnode1 = Configuration.getByPath("testng.conf")
      .getStringList("fullnode.ip.list").get(1);
  protected long maxFeeLimit = Configuration.getByPath("testng.conf")
  ...
  public long getEnergyFee() {
    ...
    return getChainParameters.get().getChainParameter(11).getValue();
  }
  ...
  public long getUserAvailableEnergy(byte[] userAddress) {
    ...
    return balance / getEnergyFee() + userAvaliableFrozenEnergy;
  }
  ...
  public long getUserMax(byte[] userAddress, long feelimit) {
    Account info = PublicMethed.queryAccount(userAddress, blockingStubFull);
    logger.info("User feeLimit: " + feelimit / getEnergyFee());
    logger.info("User UserAvaliableEnergy: " + getUserAvailableEnergy(userAddress));
    return Math.min(feelimit / getEnergyFee(), getUserAvailableEnergy(userAddress));
  }
  ...
  public void testOriginEnergyLimit() {
    ...
    long triggerFeeLimit = 1000;
    boolean expectRet = false;
    ...
    Assert.assertEquals(infoById.get().getFee(), devBalanceBefore - devBalanceAfter);
    ...
    Assert.assertTrue(devEnergyUsageBefore - devEnergyUsageAfter <= originEnergyUsage);
    Assert.assertTrue(devEnergyUsageBefore - devEnergyUsageAfter > originEnergyUsage - 100);
